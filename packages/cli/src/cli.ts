...
export async function main(originalArgs: readonly string[]): Promise<void> {
  ...
  const visiblePackages = (await installedPackages()).filter(
    (name) => name.startsWith("@cosmjs-rn/") || name === "axios",
  );
  ...
  if (args.selftest) {
    ...
    init += `
      ...
      import {
        ...
        toUtf8,
        Bech32,
      } from "@cosmjs-rn/encoding";
      import { sha512, Bip39, Random } from "@cosmjs-rn/crypto";
      import {
        coins,
        encodeAminoPubkey,
        encodeBech32Pubkey,
      ...
      import { Decimal } from "@cosmjs-rn/math";
      import { assert, arrayContentEquals, sleep } from "@cosmjs-rn/utils";
